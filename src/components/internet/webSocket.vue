<template>
  <div class="webSocket_warp">
    <el-collapse accordion>
      <el-collapse-item title="1. WebSocket 是什么">
        ...
        <p><strong>WebSocket原理</strong> ：客户端向 WebSocket 服务器通知（notify）一个带有所有接收者ID（recipients IDs）的事件（event），服务器接收后立即通知所有活跃的（active）客户端，只有ID在接收者ID序列中的客户端才会处理这个事件。
          <a name="user-content-lFocj"></a></p>
      </el-collapse-item>
      <el-collapse-item title="2. WebSocket 特点">
        <ul>
          <li>支持双向通信，实时性更强</li>
          ...
          <li>与 HTTP 协议有着良好的兼容性。默认端口也是80和443，并且握手阶段采用 HTTP 协议，因此握手时不容易屏蔽，能通过各种 HTTP 代理服务器。
            <a name="user-content-SXFiA"></a></li>
        </ul>
      </el-collapse-item>
      <el-collapse-item title="3. Websocket的使用">
        <p>在客户端中：</p>
        <pre><code class="hljs language-javascript copyable" lang="javascript"><span class="hljs-comment">// 在index.html中直接写WebSocket，设置服务端的端口号为 9999</span>
<span class="hljs-keyword">let</span> ws = <span class="hljs-keyword">new</span> WebSocket(<span class="hljs-string">'ws://localhost:9999'</span>);
...
ws.onmessage = <span class="hljs-function"><span class="hljs-keyword">function</span>(<span class="hljs-params">res</span>) </span>{
    <span class="hljs-built_in">console</span>.log(res);       <span class="hljs-comment">// 打印的是MessageEvent对象</span>
    <span class="hljs-built_in">console</span>.log(res.data);  <span class="hljs-comment">// 打印的是收到的消息</span>
};
...
<span class="copy-code-btn">复制代码</span></code></pre>
      ...
      <el-collapse-item title="4. 即时通讯的实现：短轮询、长轮询、SSE 和 WebSocket 间的区别？">
        ...
        <p><strong>上面的四个通信协议，前三个都是基于HTTP协议的。</strong>
          **
