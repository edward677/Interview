<template>
  <div class="browserSecurity_warp">
    <el-collapse accordion>
      <el-collapse-item title="1.  什么是XSS ？">
        <h4 data-id="heading-3">（1）概念</h4>
        ...
        <p>攻击者可以通过这种攻击方式来进行以下操作：</p>
        <ul>
          <li>获取页面的数据，如DOM，包括DOM、cookie、localStorage，</li>
          <li>DOS攻击，发送合理请求，占用服务器资源，从而使用户无法访问服务器</li>
          <li>破坏页面结构</li>
          <li>流量劫持（将链接指向某网站）</li>
        </ul>
        <h4 data-id="heading-4">（2）攻击类型</h4>
      ...
      <el-collapse-item title="2. 如何防御XSS？">
        <p>可以看到XSS危害如此之大， 那么在开发网站时就要做好防御措施，具体措施如下：</p>
        <ul>
          <li>可以从浏览器的执行来进行预防，一种是使用纯前端的方式，不用服务器端拼接后返回。另一种是对需要插入到 HTML 中的代码做好充分的转义。对于 DOM 型的攻击，主要是前端脚本的不可靠而造成的，我们对于数据获取渲染和字符串拼接的时候应该对可能出现的恶意代码情况进行判断。</li>
          <li>使用 CSP ，CSP 的本质是建立一个白名单，告诉浏览器哪些外部资源可以加载和执行，从而防止恶意代码的注入攻击。</li>
        </ul>
        <blockquote>
          <ol>
            <li>CSP 指的是内容安全策略，它的本质是建立一个白名单，告诉浏览器哪些外部资源可以加载和执行。我们只需要配置规 &nbsp; &nbsp;则，如何拦截由浏览器自己来实现。</li>
      ...
      <el-collapse-item title="3. 什么是CSRF ？">
        <h4 data-id="heading-7">（1）概念</h4>
        ...
        <h4 data-id="heading-8">（2）攻击类型</h4>
        <p>一般的 CSRF 有三种：</p>
        <ul>
          <li>GET 类型的 CSRF 攻击，比如在网站中的一个 img 标签里构建一个请求，当用户打开这个网站的时候就会自动发起提交。</li>
          <li>POST 类型的 CSRF 攻击，比如说构建一个表单，然后隐藏它，当用户进入页面时，自动提交这个表单。</li>
          <li>链接类型的 CSRF 攻击，比如说在 a 标签的 href 属性里构建一个请求，然后诱导用户去点击。</li>
        </ul>
      </el-collapse-item>
      <el-collapse-item title="4. 如何防御CSRF">
        <p><strong>CSRF 可以用下面几种方法来防护：</strong></p>
        <ul>
