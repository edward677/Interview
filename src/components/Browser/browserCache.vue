<template>
  <div class="browserCache_warp">
    <el-collapse accordion>
      <el-collapse-item title="1. 说一说浏览器的缓存机制">
        <p><strong>浏览器缓存的全过程：</strong></p>
        <ul>
          <li>浏览器第一次加载资源，服务器返回 200，浏览器从服务器下载资源文件，并缓存住资源文件与 response header 以供下次加载时对比使用；</li>
        ...
        <p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c7704f9a518c4d6daebf008f7dd3afa0~tplv-k3u1fbpfcp-zoom-1.image" alt="image.png" loading="lazy" class="medium-zoom-image">
          很多网站的资源后面都加了版本号，这样做的目的是：每次升级了 JS 或 CSS 文件后，为了防止浏览器进行缓存，强制改变版本号，客户端浏览器就会重新下载新的 JS 或 CSS 文件 ，以保证用户能够及时获得网站的最新更新。</p>
        <p>资源缓存的位置一共有 3 种，按优先级从高到低分别是：</p>
        <ol>
          <li><strong>Service Worker</strong>
            Service Worker 运行在 JavaScript 主线程之外，虽然由于脱离了浏览器窗体无法直接访问 DOM，但是它可以完成离线缓存、消息推送、网络代理等功能。</li>
          <li><strong>Memory Cache</strong>
            Memory Cache 就是内存缓存，它的效率最快，但是存活时间最短，你一关掉浏览器 Memory Cache 里的文件就被清空了。</li>
          <li><strong>Disk Cache</strong>
            Cache 资源被存储在硬盘上，存活时间比 Memory Cache 要持久很多。</li>
        </ol>
      </el-collapse-item>
      <el-collapse-item title="2. 协商缓存和强缓存的区别">
        <h4 data-id="heading-20">（1）强缓存</h4>
        ...
        <p><code>Cache-Control</code>可设置的字段值有很多，下面逐一介绍：</p>
        <ul>
        ...
        <h4 data-id="heading-21">（2）协商缓存</h4>
      ...
      <el-collapse-item title="3. 为什么需要浏览器缓存？">
      ...
      <el-collapse-item title="4. 用户点击刷新按钮或者按 F5、按 Ctrl+F5 （强制刷新）、地址栏回车有什么区别？">
        <ul>
