<template>
  <div class="processAndThread_warp">
    <el-collapse accordion>
      <el-collapse-item title="1. 进程与线程的概念">
        ...
        <p><strong>进程是运行在虚拟内存上的，虚拟内存是用来解决用户对硬件资源的无限需求和有限的硬件资源之间的矛盾的。从操作系统角度来看，虚拟内存即交换文件；从处理器角度看，虚拟内存即虚拟地址空间。</strong></p>
        ...
        <p>进程隔离就是为了使操作系统中的进程互不干扰，每一个进程只能访问自己占有的数据，也就避免出现进程 A 写入数据到进程 B 的情况。正是因为进程之间的数据是严格隔离的，所以一个进程如果崩溃了，或者挂起了，是不会影响到其他进程的。如果进程之间需要进行数据的通信，这时候，就需要使用用于进程间通信的机制了。</p>
        <p>看一下<strong>Chrome浏览器的架构图</strong>：
          <img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b53273a18b274735961ab3d9b095267b~tplv-k3u1fbpfcp-zoom-1.image" alt="" loading="lazy" class="medium-zoom-image">
          从图中可以看出，最新的 Chrome 浏览器包括：</p>
        <ul>
          <li>1 个浏览器主进程</li>
          <li>1 个 GPU 进程</li>
          <li>1 个网络进程</li>
      ...
      <el-collapse-item title="2. 进程和线程的区别">
        <ul>
          <li>进程可以看做独立应用，线程不能</li>
          <li>资源：进程是资源分配的基本单位，线程共享所属进程的资源。进程有独立的地址空间，互不影响，线程只是进程的不同执行路径，某个线程挂了其所在的进程也会挂掉（所以多进程程序会比多线程程序更加健壮）。线程没有独立的地址空间。</li>
      ...
      <el-collapse-item title="3. 浏览器渲染进程的线程">
        <p>浏览器的渲染进程的线程总共有五种：
          <img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/56f0b36edfaa4d8a862dd173c311dd24~tplv-k3u1fbpfcp-zoom-1.image" alt="" loading="lazy" class="medium-zoom-image">
          <strong>（1）GUI渲染线程</strong></p>
        <ul>
          <li>负责渲染浏览器页面，解析HTML、CSS，构建DOM树、CSSOM树、渲染树和绘制页面</li>
          <li>当界面需要<strong>重绘</strong>或由于某种操作引发<strong>回流</strong>时，该线程就会执行</li>
        </ul>
        <p>注意：GUI渲染线程和JS引擎线程是互斥的，当JS引擎执行时GUI线程会被挂起，GUI更新会被保存在一个队列中等到JS引擎空闲时立即被执行。</p>
        <p><strong>（2）JS引擎线程</strong></p>
        <ul>
          <li>JS引擎线程也称为JS内核，负责处理Javascript脚本程序，解析Javascript脚本，运行代码；</li>
          <li>JS引擎线程一直等待着任务队列中任务的到来，然后加以处理，一个Tab页中无论什么时候都只有一个JS引擎线程在运行JS程序；</li>
        </ul>
        <p>注意：GUI渲染线程与JS引擎线程的互斥关系，所以如果JS执行的时间过长，会造成页面的渲染不连贯，导致页面渲染加载阻塞。</p>
        <p><strong>（3）时间触发线程</strong></p>
        <ul>
          <li>属于浏览器而不是JS引擎，用来控制事件循环；</li>
          <li>当JS引擎执行代码块如setTimeOut时（也可是来自浏览器内核的其他线程,如鼠标点击、AJAX异步请求等），会将对应任务添加到事件触发线程中；</li>
        ...
        <ul>
          <li>即setInterval与setTimeout所在线程；</li>
          <li>浏览器定时计数器并不是由JS引擎计数的，因为JS引擎是单线程的，如果处于阻塞线程状态就会影响记计时的准确性；</li>
          <li>因此使用单独线程来计时并触发定时器，计时完毕后，添加到事件队列中，等待JS引擎空闲后执行，所以定时器中的任务在设定的时间点不一定能够准时执行，定时器只是在指定时间点将任务添加到事件队列中；</li>
        </ul>
